...
本記事では環境構築に以下の2つのツールを使います。元々は別のツールですが、両者を連携させることで、仮想環境の構築で必要となる一連の作業（仮想マシンの立ち上げ、OSや各種ソフトウェアのインストール、アプリケーションのビルド）を自動化できます。

- Vagrant: 仮想マシンのフロントエンドとなるツール。指定した設定の仮想マシンを立ち上げることができる。また、有志によるプラグインを利用することで、立ち上げた仮想マシンに対して Chef をインストール・実行できる。
- Chef: ソフトウェアのインストールや各種設定を行うことができる。本記事では、ゲストOSの環境設定に利用する。
