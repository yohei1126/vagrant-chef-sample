...
本記事では Vagrant と Chef というツールを使って、コードを使ってインフラの構築を自動的に行う手順を示します。

# コードによる環境構築の自動化

AWSといったクラウドサービスが一般に浸透したことにより、仮想環境を前提とした開発が一般的になってきました。仮想環境が前提になった開発では、必要になったときに必要な構成のインスタンスを立ち上げることが一般的であり、以前よりも頻繁にインスタンスを構築し直す機会が増えてきています。このような状況では、環境構築をより効率的に行うことが重要になり、コードによって環境構築手順を自動化する動きが盛んになってきています。

コードによる環境構築の効果として、以下のようなことが挙げられます。
- 環境構築手順がコードになっており、何度でも同じ環境が構築できます。作業ミスによって環境に差分が生まれ、システムの誤動作を防ぐことができます。
- 個人の暗黙的なノウハウに依存していた部分が形式知化できます。

そこで本記事では、インフラ構築自動化の例として、VM 上の CentOS 7 に Play Framework と PostgreSQL を使ったアプリケーション環境を構築します。ツールとしては以下を利用します。

- Vagrant: 仮想マシンの立ち上げ・設定を行う
- Chef: 立ち上げた仮想マシンに対してソフトウェアのインストール他、各種設定を行う
